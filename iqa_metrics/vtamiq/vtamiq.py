...
from .VisionTransformer.backbone import VisionTransformerBackbone
from .VisionTransformer.transformer import LayerScale
from .RCAN.channel_attention import ResidualGroup
...
class VTAMIQ(VisionTransformerBackbone):
    def __init__(
        ...
        if calibrate:
            self.quality_decoder = get_quality_decoder(
                ...
                rg_path_drop=rg_path_drop, use_ms_cam=False, use_local=False)
        else:
    ...
    def forward(self, patches, pos, scales):
        ...
        # .view(B, -1, 1) reshapes to B x H x 1
        cls_diff = self.quality_decoder(cls_diff.view(B, -1, 1))
