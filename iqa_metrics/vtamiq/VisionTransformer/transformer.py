...
WEIGHTS_PATH = './iqa_metrics/vtamiq/VisionTransformer/weights'
...
def get_B16_config():
    vit_b16_config = OrderedDict(
        vit_weights_path=f"{WEIGHTS_PATH}/imagenet21k+imagenet2012_ViT-B_16.npz",
        img_dim=384,
        patch_size=16,
        hidden_size=768,
        mlp_dim=3072,
...
def get_B8_config():
    ...
    vit_b8_config["vit_weights_path"] = f"{WEIGHTS_PATH}/imagenet21k+imagenet2012_ViT-B_8.npz"
    return vit_b8_config


def get_L16_config():
    vit_l16_config = OrderedDict(
        vit_weights_path=f"{WEIGHTS_PATH}/imagenet21k+imagenet2012_ViT-L_16.npz",
        img_dim=384,
        patch_size=16,
        hidden_size=1024,
        mlp_dim=4096,
...
class MultiHeadSelfAttention(nn.Module):
    def __init__(self, config, num_tokens, attn_bias_weight=0.0, attn_bias_detach=True):
        ...
        self.attn_bias_weight = max(0.0, min(1.0, attn_bias_weight))
    ...
    def forward(self, x, return_attention=True, attn_bias=None):
        q, k, v = self.compute_qkv(x)

        attn_scores = torch.matmul(q, k.transpose(-1, -2)) / math.sqrt(self.attention_head_size)

        # apply attention bias
        if attn_bias is not None and 0.0 < self.attn_bias_weight:
            if self.attn_bias_detach:
                attn_bias = attn_bias.detach()
            attn_bias /= attn_bias.max()
            # print('attn_bias', attn_bias.min(), attn_bias.mean(), attn_bias.max())
            attn_bias = torch.lerp(torch.ones_like(attn_bias), attn_bias, self.attn_bias_weight).unsqueeze(1)
            attn_bias = torch.zeros_like(attn_bias)
            attn_scores[:, :, 0, self.num_tokens:] *= attn_bias
            # attn_prob = torch.lerp(attn_prob, attn_bias, self.attn_bias_weight)
...
class EncoderLayer(nn.Module):
    def __init__(self, config, use_layer_scale, num_adapters, num_tokens, path_drop_prob, attn_bias_weight):
        ...
        self.ffn = MLP(config)
        self.attn = MultiHeadSelfAttention(config, num_tokens, attn_bias_weight=attn_bias_weight)
    ...
    def forward(self, x, adapter_num=-1, return_attention=True, attn_bias=None):
        h, weights = self.attn.forward(self.attention_norm(x), return_attention=return_attention, attn_bias=attn_bias)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, adapter_num=-1, return_attention=True, return_layers=True, attn_bias=None):
        attn_weights = []
        layer_states = []

        for layer in self.layers:  # type: EncoderLayer
            x, weights = layer.forward(x, adapter_num=adapter_num, attn_bias=attn_bias)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, pos, scales, tokens_only=True, adapter_num=-1, attn_bias=None):
        x = self.embeddings(x, pos, scales)  # this will transform BxNxCxPxP into (B, num_tokens+N, H)
        x, attn_weights, hidden_states = self.encoder.forward(x, adapter_num, attn_bias=attn_bias)
