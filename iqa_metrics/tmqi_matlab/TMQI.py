...
# NOTE: Modified by Andrei Chubarau to include finer control of whether modified TMQI is used
...
class TMQI(Metric):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.original_rescale = True
        self.original_naturalness = True
        self.original_naturalness_weight = True
    ...
    def _TMQI_gray(self, hdrImage, ldrImage, window=None):
        ...
        if self.original_naturalness_weight:
            # Naturalness should be calculated before rescaling
            N = self._StatisticalNaturalness(L_ldr)
        else:
            N = None

        # The images should have the same dynamic ranges
        factor = float(2 ** 16 - 1.)
        if self.original_rescale:
...
class TMQIr(TMQI):
    def __init__(self, *args, **kwargs):
        ...
        self.original_rescale = False
        self.original_naturalness = False
        self.original_naturalness_weight = True
