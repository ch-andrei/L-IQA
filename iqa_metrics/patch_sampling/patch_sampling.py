...
class PatchSampler(object):
    __centerbias_image_path__ = "./iqa_metrics/patch_sampling/centerbias.npy"
    ...
    def __init__(
        ...
        if grid_type == GRID_TYPE_PERTURBED_SIMPLE:
            if 0 < centerbias_weight or 0 < diff_weight:
                print("WARNING: PatchSampler with GRID_TYPE_PERTURBED_SIMPLE but centerbias_weight or diff_weight are non-zero. Will use uniform sampling.")
            centerbias_weight = 0
            diff_weight = 0
...
def compute_patch_num_scales(patch_num_scales, h, w, ho, wo):
    ...
    if 1 < patch_num_scales:
        ...
        patch_num_scales = max(1, min(patch_num_scales_max - 1, patch_num_scales))
    else:
        patch_num_scales = 1
...
def get_iqa_patches(imgs: (tuple, list),
    ...
    random_seed = TemporaryNumpySeed(random_seed)
    random_seed.__enter__()  # this avoids extra indent
    ...
    def get_tensors_patches(k, tensor, patch_indices):
    ...
    scales = torch.zeros((num_imgs, patch_count), dtype=torch.int)
    ...
    for scale in range(patch_num_scales):
        ...
        for k, tensor in enumerate(tensors):
            get_tensors_patches(k, tensor, patch_indices_s)
    ...
    random_seed.__exit__(None, None, None)
